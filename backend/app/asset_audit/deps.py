"""Integration seam between the Asset Audit module and the shared foundation.

The Asset Audit module is a self-contained vertical slice. It does **not** own the
declarative ``Base``, the database session, JWT/auth, or the ``users`` / ``assets``
tables — those belong to the shared foundation (Person 1). We consume them when
present and fall back to a local, standalone setup otherwise, so this package
stays importable, runnable and unit-testable in complete isolation with zero hard
coupling to files we do not own.
"""
...
from app.asset_audit.exceptions import AuthenticationError, PermissionDeniedError
