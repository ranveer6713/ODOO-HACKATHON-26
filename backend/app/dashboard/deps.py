"""Integration seam between the Dashboard module and the shared foundation.

The Dashboard is the presentation / aggregation layer. It owns **no** tables of
its own: every number it reports is aggregated read-only from tables owned by
sibling modules (Booking, Maintenance, Asset Audit, Notifications, Activity Log)
and the foundation-owned ``assets`` table. Because those tables live in the
single shared database wired up by the foundation (Person 1), the Dashboard only
functions when it is handed a session over that shared database.

Like every sibling slice it consumes the foundation's ``Base`` / ``get_db`` /
auth when present and falls back to a local, standalone setup otherwise so the
package stays importable in isolation. The standalone fallback exists purely for
import-safety and unit testing — a dashboard wired to its own empty ``dashboard``
database has nothing to aggregate; production always supplies the shared session.
"""
...
from app.dashboard.exceptions import AuthenticationError, PermissionDeniedError
