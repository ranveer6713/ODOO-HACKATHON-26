"""Integration seam between the Notifications module and the shared foundation.

The Notifications module is a self-contained vertical slice. It does **not** own
the declarative ``Base``, the database session, or JWT/auth — those belong to the
shared foundation (Person 1). We consume them when present and fall back to a
local, standalone setup otherwise, so this package stays importable, runnable and
unit-testable in complete isolation with zero hard coupling to files we do not own.
"""
...
from app.notifications.exceptions import AuthenticationError, PermissionDeniedError
